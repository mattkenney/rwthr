...
function makeNoCache(func)
{
    return function (where, count, callback)
    {
        ...
        func(where, count, function (err, result)
        {
            if (err) return callback(err);
...
function parseRadarLocations(data, callback)
{
    data = data && data.map && data.map(function (station)
    {
        return {
            ...
            url: 'https://radar.weather.gov/ridge/lite/N0R/' + station.ICAO.substring(1) + '_loop.gif'
        };
    });
    callback(null, data);
}
...
module.exports = {
    ...
    radar: makeNoCache(makeLocator(readRadarLocations)),
